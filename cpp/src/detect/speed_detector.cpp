...
void processMessage(const std::string& msg) {
    std::cout << "[BT RX] " << msg << std::endl;

    float speed; bool inLane;
    if (!parseBTMessage(msg, speed, inLane)) {
        std::cerr << "[ERROR] Failed to parse BT message\n";
        return;
    }
    long long now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(
                           std::chrono::system_clock::now()
    ...
    if (id >= 0) {
        std::cout << "[SPEEDING] Car " << id
                  << " at " << now_ms
                  << ", speed=" << speed
                  << ", sensor=" << (inLane ? "IN" : "OUT")
                  << std::endl;
    } else {
        std::cout << "[WARN] No match (sensor=" << (inLane ? "IN" : "OUT") << ")\n";
    }
}
...
int main() {
    ...
    // termios 설정: 9600, 8N1, no echo, no canonical
    struct termios opts;
