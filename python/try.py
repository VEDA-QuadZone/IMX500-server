    ...
    with MappedArray(request, stream) as m:
        ...
        for det in dets:
            ...
            if det.conf > prev_conf:
                x, y, w, h = det.box
                # "person"이면 전체 프레임, 아니면 crop
                if get_labels()[det.category] == "person":
                    snap = raw  # 전체 프레임 저장
                else:
                    snap = raw[y:y+h, x:x+w]  # crop 저장
                ...
                try:
                    if len(snap.shape) == 3:
                        if snap.shape[2] == 4:
                            ...
                            print(f"[!] 알 수 없는 채널 수: {snap.shape}")
                            continue
                    else:
                        print(f"[!] 예상치 못한 snapshot shape: {snap.shape}")
                        continue
                except Exception as e:
                    print(f"[!] snapshot 처리 중 오류: {e}")
                    continue

                # 🔥 기존 snapshot_{id}_* 삭제
