...
def main():
    ...
    # 2) preview configuration 적용
    cfg = picam2.create_preview_configuration(
        main={"size": (1280, 720)}
        # 필요시 lores 스트림 추가:
        # , lores={"size": (640, 360)}, display="lores"
    )
    picam2.configure(cfg)

    # 3) GPU 가속 프리뷰 시작 (Preview.QTGL 또는 Preview.DRM)
    ...
    # 5) H.264 하드웨어 인코더로 녹화 시작
    #    (출력 파일명과 옵션은 필요에 따라 조정)
    picam2.start_recording(encoder, "output.h264")

    try:
        # 메타-프레임 링버퍼 유지를 위한 루프
        while True:
            time.sleep(1)
